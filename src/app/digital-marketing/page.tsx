...
function LeadForm({ dark = false }: { dark?: boolean }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      trackMetaCapiEvent({
        ...
        customData: {
          content_name: 'Digital Marketing Landing Page Form',
          lead_type: 'Digital Marketing Doctor Lead',
      ...
      if (res.ok) { setStatus('success'); setFormData({ name: '', whatsapp: '', specialty: '', city: '' }); window.location.href = 'https://calendly.com/jaydeepkataria/30min'; }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <input type="text" name="name" placeholder="Your Name (Dr. ___) *" required value={formData.name} onChange={handleChange} className={inputClass} />
